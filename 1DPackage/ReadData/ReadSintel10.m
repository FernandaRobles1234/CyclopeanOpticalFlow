...
im=ImageData[ia];
imdis=ImageData[id];
pos=Flatten[Table[{x+imdis[[y,x]]*0.1,46-y}->im[[y,x]],{y,1,45},{x,1,103}]];

im=ImageData[ia];
imdis=ImageData[id];
pos=Flatten[Table[{x+imdis[[y,x]]*0.1,ImageDimensions[ia][[2]]+1-y}->im[[y,x]],{y,1,ImageDimensions[ia][[2]]},{x,1,ImageDimensions[ia][[1]]}]];

SetSystemOptions["MaxPatternMatchRecursion"->Infinity];
iaa=ReplacePixelValue[ia,pos[[1;;-1]]];

{ia,iaa,Image[imdis*0.1],io}
)];
